...
import MemberInfo from '../../../src/header/userInfo/MemberInfo';
...
describe('', () => {
    ...
    test('로그아웃 상태에선 로그인버튼이 출력됩니다.', async () => {
        const { container } = render(<MemberInfo isLoggedIn={false} />);
    ...
    test('로그인 버튼을 누르면 구글 로그인 페이지로 리다이렉트됩니다.', async () => {
        const { container } = render(<MemberInfo isLoggedIn={false}></MemberInfo>);
    ...
    test('로그인 상태에선 사용자정보와 로그아웃버튼이 출력됩니다.', async () => {
        const { container } = render(<MemberInfo isLoggedIn={true} />);
